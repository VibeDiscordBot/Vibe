import { StreamDispatcher, VoiceChannel, VoiceConnection } from "discord.js-light";
...
export default class Player {
    ...
    private async continue() {
        if(this.status === PlayerStatus.Connected || this.status === PlayerStatus.Playing) {
    ...
    public disconnect() {
        Logger.info(`Disconnecting from ${this?.channel.guild.name}:${this?.channel.name}`)
